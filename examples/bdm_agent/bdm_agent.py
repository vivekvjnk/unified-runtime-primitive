...
logger = logging.getLogger("urp.bdm_agent")
...
class BDMURPAgent(AbstractURPAgent):
    ...
    def _conversation_callback(self, event: Event):
            ...
            try:
                ...
                logger.info(f"[BDMURPAgent] Conversation Event: {event_dict}")
            except Exception:
    ...
    def _on_initialize(self, context: Any) -> None:
        ...
        sys_prompt_path = os.path.join(submodule_root, "bdm_prompt.j2")
        ...
        if conv_id_str:
            try:
                ...
                logger.info(f"[BDMURPAgent._on_initialize] Resuming conversation with ID: {conversation_id}")
            except ValueError:
                conversation_id = uuid.uuid4()
                logger.warning(f"[BDMURPAgent._on_initialize] Invalid conversation ID provided, created new: {conversation_id}")
        else:
            conversation_id = uuid.uuid4()
            logger.info(f"[BDMURPAgent._on_initialize] Created new conversation with ID: {conversation_id}")
        ...
        logger.info(f"[BDMURPAgent] Initialized with workspace: {self.workspace_path}")
    ...
    async def process(self, message: MessageEnvelope) -> ProcessResult:
        ...
        """
        logger.info(f"[BDMURPAgent] Processing message: {message}")
